...
def conv(layer_name, x, out_channels, kernel_size=[3,3], stride=[1,1,1,1], is_pretrain=True):
    ...
    Args:
        layer_name: e.g. conv1, pool1...给这一层一个名字
        x: input tensor, [batch_size, height, width, channels]
        out_channels: number of output channels (or comvolutional kernels)
        kernel_size: the size of convolutional kernel, VGG paper used: [3,3]
        stride: A list of ints. 1-D of length 4. VGG paper used: [1, 1, 1, 1]
        is_pretrain: if load pretrained parameters, freeze all conv layers. True：加载参数，不从0开始训练
    ...
    with tf.variable_scope(layer_name):
        w = tf.get_variable(name='weights',
                            ...
                            initializer=tf.contrib.layers.xavier_initializer()) # default默认 is uniform distribution initialization
...
def pool(layer_name, x, kernel=[1,2,2,1], stride=[1,2,2,1], is_max_pool=True):
    ...
    Args:
        ...
        padding:
        is_max_pool: boolen #还有一种池化方式：平均池化
...
#%%
def batch_norm(x): #加上bn网络收敛的非常快非常好
...
def FC_layer(layer_name, x, out_nodes):
    ...
    with tf.variable_scope(layer_name):
        ...
        x = tf.nn.bias_add(tf.matmul(flat_x, w), b) #矩阵相乘
        x = tf.nn.relu(x)
        return x
